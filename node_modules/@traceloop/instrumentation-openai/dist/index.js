...
function buildOpenAIResponsesOutputMessage(result) {
    var _a, _b;
    let finishReason = aiSemanticConventions.FinishReasons.STOP;
    if (result.status === "completed") {
        finishReason = aiSemanticConventions.FinishReasons.STOP;
    }
    else if (result.status === "incomplete") {
        const reason = (_a = result.incomplete_details) === null || _a === void 0 ? void 0 : _a.reason;
        if (reason === "max_output_tokens") {
            finishReason = aiSemanticConventions.FinishReasons.LENGTH;
        }
        else if (reason === "content_filter") {
            finishReason = aiSemanticConventions.FinishReasons.CONTENT_FILTER;
        }
        else {
            finishReason = aiSemanticConventions.FinishReasons.ERROR;
        }
    }
...
class OpenAIInstrumentation extends instrumentation.InstrumentationBase {
            ...
            return function method(...args) {
                ...
                if (type === RESPONSES_TYPE) {
                    if (isStream) {
                        ...
                        return api.context.bind(execContext, execPromise);
                    }
    ...
    _startResponsesSpan(params, client) {
        ...
        const attributes = {
            [incubating.ATTR_GEN_AI_PROVIDER_NAME]: provider,
            [incubating.ATTR_GEN_AI_OPERATION_NAME]: incubating.GEN_AI_OPERATION_NAME_VALUE_CHAT,
        };
        try {
            attributes[incubating.ATTR_GEN_AI_REQUEST_MODEL] = params.model;
            if (params.max_output_tokens != null) {
                attributes[incubating.ATTR_GEN_AI_REQUEST_MAX_TOKENS] = params.max_output_tokens;
            }
        ...
        return this.tracer.startSpan(`${incubating.GEN_AI_OPERATION_NAME_VALUE_CHAT} ${params === null || params === void 0 ? void 0 : params.model}`, {
            kind: api.SpanKind.CLIENT,
            attributes,
        });
    }
        ...
        try {
            ...
            if (result.usage) {
                ...
                if (typeof inputTokens === "number") {
                    span.setAttribute(incubating.ATTR_GEN_AI_USAGE_INPUT_TOKENS, inputTokens);
                }
                if (typeof outputTokens === "number") {
                    span.setAttribute(incubating.ATTR_GEN_AI_USAGE_OUTPUT_TOKENS, outputTokens);
                }
