...
const classNameToProviderName = {
    OpenAI: incubating.GEN_AI_PROVIDER_NAME_VALUE_OPENAI,
...
const openAIFinishReasonMap = {
    ...
    function_call: aiSemanticConventions.FinishReasons.TOOL_CALL,
};
class CustomLLMInstrumentation {
    ...
    chatWrapper({ className }) {
        ...
        return (original) => {
            return function method(...args) {
                ...
                try {
                    span.setAttribute(incubating.ATTR_GEN_AI_PROVIDER_NAME, (_a = classNameToProviderName[className]) !== null && _a !== void 0 ? _a : className.toLowerCase());
                    span.setAttribute(incubating.ATTR_GEN_AI_REQUEST_MODEL, this.metadata.model);
                    span.setAttribute(incubating.ATTR_GEN_AI_OPERATION_NAME, incubating.GEN_AI_OPERATION_NAME_VALUE_CHAT);
                    span.setAttribute(incubating.ATTR_GEN_AI_REQUEST_TOP_P, this.metadata.topP);
                    span.setAttribute(incubating.ATTR_GEN_AI_REQUEST_TEMPERATURE, this.metadata.temperature);
                    if (shouldSendPrompts(plugin.config()) && messages) {
                        span.setAttribute(incubating.ATTR_GEN_AI_INPUT_MESSAGES, instrumentationUtils.formatInputMessages(messages, instrumentationUtils.mapOpenAIContentBlock));
                    }
                }
                catch (e) {
                    plugin.diag.warn(e);
    ...
    handleResponse(result, span, metadata) {
        ...
        span.setAttribute(incubating.ATTR_GEN_AI_RESPONSE_MODEL, metadata.model);
        try {
            const raw = result.raw;
            if (raw === null || raw === void 0 ? void 0 : raw.id) {
                span.setAttribute(incubating.ATTR_GEN_AI_RESPONSE_ID, raw.id);
            }
            ...
            if (finishReason != null) {
                span.setAttribute(incubating.ATTR_GEN_AI_RESPONSE_FINISH_REASONS, [
            ...
            if (usage) {
                span.setAttribute(incubating.ATTR_GEN_AI_USAGE_INPUT_TOKENS, usage.prompt_tokens);
                span.setAttribute(incubating.ATTR_GEN_AI_USAGE_OUTPUT_TOKENS, usage.completion_tokens);
                span.setAttribute(aiSemanticConventions.SpanAttributes.GEN_AI_USAGE_TOTAL_TOKENS, usage.total_tokens);
            }
            ...
            if (shouldSendPrompts(this.config()) &&
                ...
                span.setAttribute(incubating.ATTR_GEN_AI_OUTPUT_MESSAGES, instrumentationUtils.formatOutputMessage(contentArray, finishReason, openAIFinishReasonMap, incubating.GEN_AI_OPERATION_NAME_VALUE_CHAT, instrumentationUtils.mapOpenAIContentBlock));
            }
            span.setStatus({ code: api.SpanStatusCode.OK });
        }
        catch (e) {
        ...
        return llmGeneratorWrapper(result, execContext, (message, lastChunk) => {
            ...
            try {
                ...
                if (lastRaw === null || lastRaw === void 0 ? void 0 : lastRaw.id) {
                    span.setAttribute(incubating.ATTR_GEN_AI_RESPONSE_ID, lastRaw.id);
                }
                ...
                if (shouldSendPrompts(this.config())) {
                    span.setAttribute(incubating.ATTR_GEN_AI_OUTPUT_MESSAGES, instrumentationUtils.formatOutputMessage([message], finishReason, openAIFinishReasonMap, incubating.GEN_AI_OPERATION_NAME_VALUE_CHAT, instrumentationUtils.mapOpenAIContentBlock));
                }
            }
            catch (e) {
                this.diag.warn(e);
