...
const require_tracer = require("./tracer-OWWxfDia.cjs");
...
const LATITUDE_CONTEXT_KEY = (0, _opentelemetry_api.createContextKey)("latitude-internal-context");
...
function capture(name, fn, options = {}) {
	...
	const parentContext = shouldReuseTrace ? currentContext : _opentelemetry_api.trace.deleteSpan(currentContext);
	...
	if (_opentelemetry_api.trace.getSpan(currentContext) && shouldReuseTrace) return _opentelemetry_api.context.with(newContext, fn);
	return _opentelemetry_api.trace.getTracer(CAPTURE_TRACER_NAME).startActiveSpan(name, { attributes: { "latitude.capture.root": true } }, newContext, (span) => {
		let result;
		try {
			result = fn();
		} catch (error) {
...
const INTEGRATIONS = {
	openai: {
		loadCtor: async () => (await Promise.resolve().then(() => require("./instrumentation-Cyvv2weP.cjs"))).OpenAIInstrumentationWithResponses,
		packageName: "@traceloop/instrumentation-openai",
		normalize: normalizeOpenAI,
		enrichTokens: true
	},
	"openai-agents": {
		loadCtor: async () => (await Promise.resolve().then(() => require("./instrumentation-BcA69tg8.cjs"))).OpenAIAgentsInstrumentation,
...
var ServiceNameResourceExporter = class {
	...
	constructor(inner, serviceName) {
		this.inner = inner;
		this.overlay = (0, _opentelemetry_resources.resourceFromAttributes)({ [_opentelemetry_semantic_conventions.ATTR_SERVICE_NAME]: serviceName });
	}
...
var LatitudeSpanProcessor = class {
	...
	constructor(apiKey, project, options) {
		...
		const baseExporter = options?.exporter ?? new _opentelemetry_exporter_trace_otlp_http.OTLPTraceExporter({
		...
		const batchOrSimple = options?.disableBatch ? new _opentelemetry_sdk_trace_node.SimpleSpanProcessor(exporter) : new _opentelemetry_sdk_trace_node.BatchSpanProcessor(exporter);
...
function getRegisteredTracerProvider() {
	const provider = _opentelemetry_api.trace.getTracerProvider();
	if (provider instanceof _opentelemetry_api.ProxyTracerProvider) return provider.getDelegateTracer(DETECT_PROBE) === void 0 ? void 0 : provider.getDelegate();
	return provider;
}
...
var Latitude = class {
	...
	constructor(options) {
		...
		if (targetProvider && attached) {
			...
			const latitudeProvider = new _opentelemetry_sdk_trace_node.NodeTracerProvider({
				resource: (0, _opentelemetry_resources.resourceFromAttributes)({ [_opentelemetry_semantic_conventions.ATTR_SERVICE_NAME]: resourceServiceName }),
	...
	async shutdown() {
		if (this.ownsProvider && this.provider instanceof _opentelemetry_sdk_trace_node.NodeTracerProvider) {
...
//#endregion
exports.ExportFilterSpanProcessor = ExportFilterSpanProcessor;
exports.Latitude = Latitude;
exports.LatitudeSpanProcessor = LatitudeSpanProcessor;
exports.RedactSpanProcessor = RedactSpanProcessor;
exports.RedactThenExportSpanProcessor = RedactThenExportSpanProcessor;
exports.buildShouldExportSpan = buildShouldExportSpan;
exports.buildShouldExportSpanFromFields = buildShouldExportSpanFromFields;
exports.capture = capture;
exports.getLatitudeTracer = require_tracer.getLatitudeTracer;
exports.initLatitude = initLatitude;
exports.isDefaultExportSpan = isDefaultExportSpan;
exports.isGenAiOrLlmAttributeSpan = isGenAiOrLlmAttributeSpan;
exports.isLatitudeInstrumentationSpan = isLatitudeInstrumentationSpan;
exports.registerLatitudeInstrumentations = registerLatitudeInstrumentations;
