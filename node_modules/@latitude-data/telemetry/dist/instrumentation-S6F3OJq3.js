...
import { i as deriveFinishReason, n as buildMessagesFromList, r as buildOutputMessages, t as buildInputMessages } from "./messages-5-b5tIgg.js";
import { SpanKind, SpanStatusCode, context, trace } from "@opentelemetry/api";
import { InstrumentationBase } from "@opentelemetry/instrumentation";
//#region src/sdk/instrumentations/openai-agents/instrumentation.ts
/**
...
*/
var OpenAIAgentsTraceProcessor = class {
	tracer;
	traces = /* @__PURE__ */ new Map();
	spans = /* @__PURE__ */ new Map();
	constructor(tracer = getLatitudeTracer("openai-agents")) {
		this.tracer = tracer;
	}
	async onTraceStart(t) {
		const parentCtx = context.active();
		const otelSpan = this.tracer.startSpan(t.name || "openai.agents.trace", {
		...
		this.traces.set(t.traceId, {
			span: otelSpan,
			ctx: trace.setSpan(parentCtx, otelSpan)
		});
	}
	async onTraceEnd(t) {
		const entry = this.traces.get(t.traceId);
		if (!entry) return;
		if (t.metadata && Object.keys(t.metadata).length > 0) entry.span.setAttribute("openai.agents.trace_metadata", safeJson(t.metadata));
		entry.span.end();
	...
	async onSpanStart(span) {
		...
		const otelSpan = this.tracer.startSpan(initialSpanName(span.spanData), {
	...
	async onSpanEnd(span) {
		...
		if (span.error) {
			...
			if (span.error.data) entry.span.setAttribute("exception.data", safeJson(span.error.data));
		}
...
function initialSpanName(data) {
	switch (data.type) {
		case "agent": return `agent ${data.name}`;
		case "function": return `function ${data.name}`;
		case "generation": return "gen_ai.generate";
		case "response": return "gen_ai.response";
		case "handoff": return "agent.handoff";
		case "guardrail": return `agent.guardrail ${data.name}`;
		case "transcription": return "audio.transcribe";
		case "speech": return "audio.speech";
		case "speech_group": return "audio.speech_group";
		case "mcp_tools": return data.server ? `mcp.list_tools ${data.server}` : "mcp.list_tools";
		case "custom": return data.name;
	}
}
function finalSpanName(data) {
	...
	if (data.type === "handoff" && (data.from_agent || data.to_agent)) return `agent.handoff ${data.from_agent ?? "?"} -> ${data.to_agent ?? "?"}`;
...
function applySpanDataAttributes(otelSpan, data) {
	switch (data.type) {
		case "agent":
		...
		case "generation":
			...
			if (data.input?.length) otelSpan.setAttribute("gen_ai.input.messages", JSON.stringify(buildMessagesFromList(data.input)));
			if (data.output?.length) otelSpan.setAttribute("gen_ai.output.messages", JSON.stringify(buildMessagesFromList(data.output)));
			if (data.model_config) otelSpan.setAttribute("gen_ai.request.config", safeJson(data.model_config));
			if (data.usage) {
				if (typeof data.usage.input_tokens === "number") otelSpan.setAttribute("gen_ai.usage.input_tokens", data.usage.input_tokens);
				if (typeof data.usage.output_tokens === "number") otelSpan.setAttribute("gen_ai.usage.output_tokens", data.usage.output_tokens);
			}
			return;
		case "response": {
			otelSpan.setAttribute("gen_ai.system", "openai");
			...
			if (response?.usage) {
				if (typeof response.usage.input_tokens === "number") otelSpan.setAttribute("gen_ai.usage.input_tokens", response.usage.input_tokens);
				if (typeof response.usage.output_tokens === "number") otelSpan.setAttribute("gen_ai.usage.output_tokens", response.usage.output_tokens);
			}
			const tools = data._response?.tools;
			if (Array.isArray(tools) && tools.length > 0) otelSpan.setAttribute("gen_ai.tool.definitions", JSON.stringify(tools));
			if (data._input !== void 0) otelSpan.setAttribute("gen_ai.input.messages", JSON.stringify(buildInputMessages({ input: data._input })));
			if (response) {
		...
		case "custom":
			...
			if (data.data && Object.keys(data.data).length > 0) otelSpan.setAttribute("openai.agents.custom.data", safeJson(data.data));
			return;
	}
}
...
//#endregion
export { OpenAIAgentsInstrumentation };

//# sourceMappingURL=instrumentation-S6F3OJq3.js.map